...
def test_cmsis_dap_tcp(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
    dut.expect('CMSIS-DAP TCP running on ESP32')
    log_minimum_free_heap_size()


@pytest.mark.host_test
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_cmsis_dap_tcp_linux(dut: IdfDut) -> None:
    dut.expect('CMSIS-DAP TCP running on ESP32')
...
@pytest.mark.macos
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_cmsis_dap_tcp_macos(dut: IdfDut) -> None:
    dut.expect('CMSIS-DAP TCP running on ESP32')
...
def test_cmsis_dap_tcp_host(app: QemuApp, dut: QemuDut) -> None:
